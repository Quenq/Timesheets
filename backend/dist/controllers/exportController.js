...
class ExportController {
    ...
    static async exportToExcel(req, res) {
        try {
            ...
            const weekSingle = req.query.week;
            const weekStart = req.query.week_start || weekSingle;
            const weekEnd = req.query.week_end || weekSingle;
            if (!weekStart) {
            ...
            const adminResult = await connection_1.default.query('SELECT company_name FROM users WHERE id = $1', [req.user.id]);
            const companyName = adminResult.rows[0]?.company_name || '';
            ...
            const sheetsResult = await connection_1.default.query(`SELECT DISTINCT p.id, p.name
         FROM timesheets t
         ...
         ORDER BY p.name`, [weekStart, weekEnd || weekStart]);
     ...
     */
    static async createSummarySheet(workbook, weekStart, weekEnd, companyName) {
        ...
        const projectsResult = await connection_1.default.query(`SELECT DISTINCT p.id, p.name, t.week_start_date, COUNT(DISTINCT tr.employee_id) as employee_count
       FROM timesheets t
       JOIN timesheet_rows tr ON tr.timesheet_id = t.id
       JOIN projects p ON tr.project_id = p.id
       WHERE t.week_start_date >= $1 AND t.week_start_date <= $2 AND t.status = 'approved'
       GROUP BY p.id, p.name, t.week_start_date
       ORDER BY t.week_start_date, p.name`, [weekStart, weekEnd]);
        let totalHours = 0;
        let totalRegularHours = 0;
        let totalOTHours = 0;
        let totalAmount = 0;
        for (const project of projectsResult.rows) {
            const ticketsResult = await connection_1.default.query(`SELECT t.hours, t.rate_type, t.amount
         FROM tickets t
         JOIN timesheet_rows tr ON t.timesheet_row_id = tr.id
         JOIN timesheets ts ON tr.timesheet_id = ts.id
         WHERE tr.project_id = $1 AND ts.week_start_date = $2 AND t.status = 'approved'`, [project.id, project.week_start_date]);
            let projectHours = 0;
     ...
     */
    static async createProjectSheet(workbook, projectName, projectId, weekStart, weekEnd) {
        ...
        const rowsResult = await connection_1.default.query(`SELECT tr.*, e.name as employee_name, t.week_start_date
       ...
       ORDER BY t.week_start_date, e.name`, [projectId, weekStart, weekEnd]);
        let totalProjectHours = 0;
        let totalProjectAmount = 0;
        for (const row of rowsResult.rows) {
            // Get tickets for this row to calculate amount
            const ticketsResult = await connection_1.default.query('SELECT SUM(amount) as total FROM tickets WHERE timesheet_row_id = $1', [row.id]);
     ...
     */
    static async createAllProjectsSheet(workbook, weekStart, weekEnd) {
        ...
        headerRow.font = { bold: true, color: { argb: 'FFFFFFFF' } };
        headerRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF366092' } };
        const projectsResult = await connection_1.default.query(`SELECT DISTINCT p.id, p.name, u.email as supervisor_email
       FROM timesheets t
       JOIN timesheet_rows tr ON tr.timesheet_id = t.id
       JOIN projects p ON tr.project_id = p.id
       JOIN users u ON t.supervisor_id = u.id
       WHERE t.week_start_date >= $1 AND t.week_start_date <= $2 AND t.status = 'approved'
       ORDER BY p.name`, [weekStart, weekEnd]);
        let totalHours = 0;
        let totalRegularHours = 0;
        let totalOTHours = 0;
        let totalAmount = 0;
        for (const project of projectsResult.rows) {
         ...
         JOIN timesheet_rows tr ON t.timesheet_row_id = tr.id
         JOIN timesheets ts ON tr.timesheet_id = ts.id
         WHERE tr.project_id = $1 AND ts.week_start_date >= $2 AND ts.week_start_date <= $3 AND t.status = 'approved'`, [project.id, weekStart, weekEnd]);
            let projectHours = 0;
            let projectRegularHours = 0;
            let projectOTHours = 0;
            let projectAmount = 0;
            ...
            // Get employee count
            const empResult = await connection_1.default.query(`SELECT COUNT(DISTINCT tr.employee_id) as count
         FROM timesheet_rows tr
         JOIN timesheets t ON tr.timesheet_id = t.id
         WHERE tr.project_id = $1 AND t.week_start_date >= $2 AND t.week_start_date <= $3`, [project.id, weekStart, weekEnd]);
            totalHours += projectHours;
            totalRegularHours += projectRegularHours;
            totalOTHours += projectOTHours;
            totalAmount += projectAmount;
