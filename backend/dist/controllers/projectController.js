...
class ProjectController {
    static async canAccessProject(req, projectId) {
        if (req.user?.role !== 'supervisor') {
            return true;
        }
        const result = await connection_1.default.query('SELECT 1 FROM project_supervisors WHERE project_id = $1 AND supervisor_id = $2', [projectId, req.user.id]);
    ...
    static async getProjects(req, res) {
        try {
            ...
            if (req.user?.role === 'supervisor') {
                ...
                result = await connection_1.default.query(`SELECT p.* FROM projects p
           JOIN project_supervisors ps ON ps.project_id = p.id
           WHERE ps.supervisor_id = $1
           ORDER BY CASE p.status WHEN 'active' THEN 1 WHEN 'on_hold' THEN 2 WHEN 'completed' THEN 3 ELSE 4 END, p.created_at DESC`, [req.user.id]);
            }
            else {
                result = await connection_1.default.query(`SELECT * FROM projects ORDER BY
    ...
    static async getProjectSupervisors(req, res) {
        try {
            const { id } = req.params;
            ...
            const result = await connection_1.default.query(`SELECT u.id, u.email, u.name FROM users u
         JOIN project_supervisors ps ON ps.supervisor_id = u.id
         WHERE ps.project_id = $1 ORDER BY u.email`, [projectId]);
            res.json(result.rows);
        }
        catch (error) {
    ...
    static async addProjectSupervisor(req, res) {
        try {
            const { id } = req.params;
            const { supervisor_id } = req.body;
            if (!supervisor_id)
                return res.status(400).json({ error: 'supervisor_id required' });
            await connection_1.default.query(`INSERT INTO project_supervisors (project_id, supervisor_id) VALUES ($1, $2)`, [id, supervisor_id]);
    ...
    static async getProjectEmployees(req, res) {
        try {
            ...
            const result = await connection_1.default.query(`SELECT e.id, e.name, e.position, e.hourly_rate FROM employees e
         JOIN employee_projects ep ON ep.employee_id = e.id
         WHERE ep.project_id = $1 ORDER BY e.name`, [projectId]);
            res.json(result.rows);
        }
        catch (error) {
    ...
    static async addProjectEmployee(req, res) {
        try {
            const { id } = req.params;
            const { employee_id } = req.body;
            if (!employee_id)
                return res.status(400).json({ error: 'employee_id required' });
            await connection_1.default.query('INSERT INTO employee_projects (employee_id, project_id) VALUES ($1, $2)', [employee_id, id]);
            res.status(201).json({ success: true });
        }
        catch (error) {
            if (error.message?.includes('UNIQUE') || error.message?.includes('unique')) {
    ...
    static async removeProjectEmployee(req, res) {
        try {
            const { id, employeeId } = req.params;
            await connection_1.default.query('DELETE FROM employee_projects WHERE project_id = $1 AND employee_id = $2', [id, employeeId]);
    ...
    static async removeProjectSupervisor(req, res) {
        try {
            const { id, supervisorId } = req.params;
            await connection_1.default.query(`DELETE FROM project_supervisors WHERE project_id = $1 AND supervisor_id = $2`, [id, supervisorId]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    static async getProjectSummary(req, res) {
        try {
            ...
            const projectId = parseInt(id);
            // Get project
            const projectResult = await connection_1.default.query('SELECT * FROM projects WHERE id = $1', [projectId]);
            ...
            const ticketsResult = await connection_1.default.query(`SELECT t.hours, t.rate_type, t.amount, t.status
         FROM tickets t
         ...
         WHERE tr.project_id = $1`, [projectId]);
            ...
            const financialSummary = (0, calculations_1.calculateProjectFinancialSummary)(project.labor_budget, project.overhead_percentage, {
                total_hours,
                ot_hours,
                total_amount,
                regular_amount,
            ...
            const supervisorsResult = await connection_1.default.query(`SELECT u.id, u.email, u.name FROM users u
         JOIN project_supervisors ps ON ps.supervisor_id = u.id
         WHERE ps.project_id = $1 ORDER BY u.email`, [projectId]);
            // Get timesheets that include rows for this project
            const timesheetsResult = await connection_1.default.query(`SELECT t.id, t.week_start_date, t.status, t.supervisor_id,
         ...
         ORDER BY t.week_start_date DESC`, [projectId]);
            // Get employee breakdown for this project
            const employeesResult = await connection_1.default.query(`SELECT e.id, e.name, e.position,
         ...
         ORDER BY SUM(tr.total_hours) DESC`, [projectId]);
            res.json({
                id: project.id,
                name: project.name,
     ...
     */
    static async getAllProjectSummaries(req, res) {
        try {
            const projectsResult = req.user?.role === 'supervisor'
                ? await connection_1.default.query(`SELECT p.* FROM projects p
             JOIN project_supervisors ps ON ps.project_id = p.id
             WHERE p.status = 'active' AND ps.supervisor_id = $1
             ORDER BY p.created_at DESC`, [req.user.id])
                : await connection_1.default.query(`SELECT * FROM projects WHERE status = 'active' ORDER BY created_at DESC`, []);
            const summaries = [];
     ...
     */
    static async buildProjectSummary(projectId, project) {
        const ticketsResult = await connection_1.default.query(`SELECT t.hours, t.rate_type, t.amount, t.status
       FROM tickets t
       JOIN timesheet_rows tr ON t.timesheet_row_id = tr.id
       WHERE tr.project_id = $1`, [projectId]);
     ...
     */
    static async getCurrentWeekHours(projectId) {
        const today = new Date();
        const dayOfWeek = today.getDay();
        const diff = today.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1);
        const monday = new Date(today.setDate(diff));
        const result = await connection_1.default.query(`SELECT SUM(tr.total_hours) as total_hours
       FROM timesheet_rows tr
       JOIN timesheets t ON tr.timesheet_id = t.id
       WHERE tr.project_id = $1 AND t.week_start_date = $2`, [projectId, monday.toISOString().split('T')[0]]);
    ...
    static async createProject(req, res) {
        try {
            ...
            if (!(0, validate_1.isNonEmptyString)(name, 200)) {
                return res.status(400).json({ error: 'Project name must be 1–200 characters' });
            }
            if (!(0, validate_1.isPositiveNumber)(labor_budget)) {
                return res.status(400).json({ error: 'labor_budget must be a positive number' });
            }
            if (overhead_percentage !== undefined && !(0, validate_1.isBetween)(overhead_percentage, 0, 200)) {
                return res.status(400).json({ error: 'overhead_percentage must be between 0 and 200' });
            }
            if (code !== undefined && code !== null && code !== '' && !(0, validate_1.isNonEmptyString)(code, 20)) {
                return res.status(400).json({ error: 'Project code must be 1–20 characters' });
            }
            const result = await connection_1.default.query(`INSERT INTO projects (name, labor_budget, overhead_percentage, status, code)
         VALUES ($1, $2, $3, $4, $5) RETURNING *`, [name, labor_budget, overhead_percentage || 20, 'active', code || null]);
    ...
    static async updateProject(req, res) {
        try {
            ...
            if (name !== undefined && !(0, validate_1.isNonEmptyString)(name, 200)) {
                return res.status(400).json({ error: 'Project name must be 1–200 characters' });
            }
            if (labor_budget !== undefined && !(0, validate_1.isPositiveNumber)(labor_budget)) {
                return res.status(400).json({ error: 'labor_budget must be a positive number' });
            }
            if (overhead_percentage !== undefined && !(0, validate_1.isBetween)(overhead_percentage, 0, 200)) {
                return res.status(400).json({ error: 'overhead_percentage must be between 0 and 200' });
            }
            const result = await connection_1.default.query(`UPDATE projects SET name = COALESCE($1, name),
         ...
         WHERE id = $6 RETURNING *`, [name, labor_budget, overhead_percentage, status, code !== undefined ? code : null, id]);
