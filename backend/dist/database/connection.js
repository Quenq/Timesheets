...
const DB_PATH = path_1.default.join(process.cwd(), 'timesheet.db');
let db = null;
async function initDatabase() {
    const SQL = await (0, sql_js_1.default)({
        locateFile: (file) => path_1.default.join(path_1.default.dirname(require.resolve('sql.js')), file),
    });
    try {
...
function execWrite(sql, params) {
    ...
    if (hasReturning && isInsert) {
        ...
        if (lastIdRows.rows.length && lastIdRows.rows[0].id) {
            insertedId = lastIdRows.rows[0].id;
        }
    }
    saveDatabase();
    if (!hasReturning)
...
const pool = {
    query: (sql, params = []) => Promise.resolve(querySync(sql, params)),
};
